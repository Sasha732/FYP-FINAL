"""
page_views/detection.py

MERGE CHANGE START
Wraps the entire AI Object Detection System (originally a standalone app.py)
as a page_views module so it can be served through the AuthFlow router.

All original YOLO code is preserved exactly as written.
Only additions are:
  - the show_detection(navigate) wrapper function
  - a "← Back to Dashboard" button in the sidebar
MERGE CHANGE END
"""
...
# ─────────────────────────────────────────
#  MERGE CHANGE START
#  Page entry point called by the router
# ─────────────────────────────────────────
def show_detection(navigate):
    # ── YOLO CSS (scoped here, not in global styles.py) ───────────────────────
    st.markdown("""
    <style>
    ...
    with st.sidebar:
        ...
        # MERGE CHANGE START — back-to-dashboard button in sidebar
        if st.button("← Dashboard", key="det_back_to_dash"):
            navigate("dashboard")
        # MERGE CHANGE END
